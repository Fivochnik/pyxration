...
- [Модуль *stringolist*, его классы, функции и методы](#модуль-stringolist-его-классы-функции-и-методы)
  - [Класс *stringolist* (строко-список) и создание его экземпляров](#класс-stringolist-строко-список-и-создание-его-экземпляров)
    ...
    - [Разбиение строко-списка на строко-списки](#разбиение-строко-списка-на-строко-списки)
...
- [Модуль *transtree*, его классы, функции и методы](#модуль-transtree-его-классы-функции-и-методы)
  - [Функции *applyUpToDown* и *applyDownToUp*](#функции-applyuptodown-и-applydowntoup)
...
##### <div align="right">[вверх↑](#оглавление)</div>
...
#### Разбиение строко-списка на строко-списки
...
Операционные деревья можно создать, если вызвать операцию с аргументами:
```python
unit = integer(1)
sum_expr = plus(unit.copy(), unit.copy(), unit.copy())
```

Это короче и выглядит лучше, чем напрямую через конструктор.
...
1. Группирующие операции в порядке, в котором первыми идут операции, открывающая и закрывающая части которых не являются подстроками открывающих и закрывающих частей других группирующих операций.
    ...
    Например, "(" и ")" являются частью "begin(" и ")end", поэтому в первую очередь обрабатываем "begin(" и ")end", а потом обычные скобки.

2. Инфиксные операции в порядке, обратном порядку выполнения математического выражения.

    Например, у нас есть строка "a+b*c+d*e+f", и мы бы хотели преобразовать его в такое выражение "plus(a, b*c, e*d, f)", а не в такое "mul(a+b, c+d, e+f)", потому что сложение выполняется после умножения, а значит первым делом строку нужно поделить на подстроки разделителем "+", чтобы не разделять слагаемые.
...
3. Префиксные операции в порядке, в котором первыми идут операции, префиксное обозначение которых не является началом префиксного обозначения других префиксных операций.
    ...
    Например, у нас есть префиксные операции, которые в строке обозначаются как "sin" и "sinh", тогда в первую очередь мы должны обработать все "sinh", чтобы не вызвалось исключения, что после "sin" стоит какой-то "h".

4. Операции-объекты в порядке, в котором другой объект не воспримется за объект текущего типа операции-объекта.

    Например, у нас есть объекты алгебры буквенная константа и переменная, буквенная константа может принимать одно из конечного числа значений (например: "e", "pi", "tau"), а переменная - любая другая строка, которая начинается с буквы или "\_" и содержит только буквы, цифры и "\_", поэтому, чтобы константное выражение "pi" не определилось, как переменная, в первую очередь пытаемся парсить его как буквенную константу.
...
Экземпляры класса *functree* - функции-деревья. Их можно создать, используя готовую алгебру, вызвав метод *new_func* со строкой в качестве параметра. Также можно подать готовый список параметров этой функции, чтобы закрепить их порядок. Остальные параметры, которых нет в списке параметров будут добавляться в этот список по мере чтения строки слева-направо. Чтобы обозначить, что данная подстрока является именем параметра, нужно обособить его процентами "%". Если в выражении нужно сохранить процент, то можно добавить "\\" перед процентом. Если нужно сохранить символ "\\" в строко-списке, можно добавить символ "\\" перед "\\".
```python
expr_str = 'x+%y%\\%z\\\\%z%'
# an_algebra - некоторая алгебра, написанная вне этого руководства.
expr = an_algebra.new_expr(expr_str)    #- функция с параметрами ['y', 'z'] вида: x+y%z\z
                                        #                                           |   |    плохой пример: имя "z" используется и для переменной и для параметра
                                        #                                         параметры
...
Не рекомендуется параметры функции-дерева называть так же, как и другие объекты (например, переменные или константные слова) в этом функции-дереве: можно запутаться в обозначениях!
...
### Функции *applyDownToUp* и *applyUpToDown*
Главные функции этого модуля, позволяют применить преобразование ко всему операционному дереву. Очень полезны тем, что пользователю не придётся разбираться с работой с деревьями, ведь эта функция сама применяет ваше преобразование к каждой части операционного дерева. Эти функции различаются лишь порядком применения преобразования. *applyDownToUp* применяет преобразование ко всему выражению, начиная с выражений, которые выполняются первыми в вашем основном выражении, а "applyUpToDown" - наоборот: начинает с внешних операций.

В эти функции подаётся операционное дерево или функция-дерево, которое нужно изменить, и функция преобразования:
```python
expr = applyDownToUp(expr, trans)
```
...
```python
expr = applyDownToUp(expr, my_transformation)
```
...
expr = applyDownToUp(expr, commutative)
```
...
, то после выполнения:
```python
expr = applyDownToUp(expr, negBinMulFormula)
```
...
```python
expr = applyDownToUp(expr, neutral_del)
```
...
```python
expr = applyDownToUp(expr, absorb)
```
...
```python
expr = applyDownToUp(expr, l_associate)
```
...
Если *expr = any_algebra.new_expr('a+b\*c\*d\*3+e+f')*, то после выполнения следующего кода:
```python
expr = applyDownToUp(expr, r_associate)
```
