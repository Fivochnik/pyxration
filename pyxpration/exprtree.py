...
class operation:
    ...
    def __call__(self, arg: 'any', /, *args: 'any'):
        #if isinstance(arg, exprtree) and all(isincstance(x, exprtree) for x in args):
        arg = [arg]
        arg.extend(args)
        return exprtree(self, arg)
        #raise TypeError('аргументы операции должны быть экземплярами "exprtree"')
...
Элементы списка должны быть кортежами из операции-группы/None и инфикс-операции/None. Если первый элемент кортежа None, то принимается любой следующий аргумент в строке. Если второй элемент кортежа None, то разбивка группы не требуется.
Например, операция "sum_of_root[n](x,y)" - сумма корней n-ой степени из аргументов "x" и "y" - имеет две группы аргументов "[]" и "()". Список будет иметь вид: "[(square_group, None), (circle_group, comma)]".
square_qroup = operation('[x]')
square_group.update_funcs(grouping_parser_and_repres('[', ']', square_group))
circle_qroup = operation('(x)')
circle_qroup.update_funcs(grouping_parser_and_repres('(', ')', circle_qroup))
comma = operation('comma')
comma.update_funcs(infix_parser_and_repres(',', comma))
        ...
        def prefix_parser(exp: 'str|stringolist') -> 'exprtree|stringolist|None':
            ...
            while True:
                ...
                for n, (group, sep) in enumerate(group_args_seps):
                    cur_arg = res[arg_pos + n].trees[0]#cur_arg = res[arg_pos + n]
                    cur_args = cur_arg.trees if isinstance(cur_arg, exprtree) else [cur_arg]#cur_args = cur_arg.trees[0].split(sep) if isinstance(cur_arg.trees[0], stringolist) else [cur_arg.trees[0]]
...
def postfix_parser_and_repres(oper_postfix: str, oper: operation, group_args_seps: list = None) -> ('func(str|stringolist) -> exprtree|stringolist|None', 'func(list) -> str'):
    """Возвращает функцию-парсер и функцию представления твоей операции.
oper_postfix - обозначение постфиксной части операции в строковом выражении;
oper - сама операция, у которой ещё нет функций парсеровки и представления;
group_args_seps - список пар из операций-групп и разделителей операций-групп, принимаемых в качестве аргумента (по-умолчанию: None).

Аргумент oper_postfix должен быть уникальной строкой, которая не является подстрокой других частей других операций, для правильной обработки строкового выражения.

Аргумент group_args_seps задаёт список пар операций-групп и разделителей аргументов внутри групп, к которым применяется данная операция. Если значение аргумента - None, операция применяется ко всему слева.
Элементы списка должны быть кортежами из операции-группы/None и инфикс-операции/None. Если первый элемент кортежа None, то принимается любой следующий аргумент в строке. Если второй элемент кортежа None, то разбивка группы не требуется.
Например, операция "[n](x,y)!" - сумма факториалов аргументов "x" и "y" по модулю "n" - имеет две группы аргументов "[]" и "()". Список будет иметь вид: "[(square_group, None), (circle_group, comma)]".
square_qroup = operation('[x]')
square_group.update_funcs(grouping_parser_and_repres('[', ']', square_group))
circle_qroup = operation('(x)')
circle_qroup.update_funcs(grouping_parser_and_repres('(', ')', circle_qroup))
...
Для создания своей постфиксной операции нужно:
1. Создать экземпляр операции;
2. Обновить его функции парсировки и представления с помощью этой функции.
Вот, как это выглядит в коде:
fact = operation('factorial')
fact.funcs_update(prefix_parser_and_repres('!', fact, [None]))"""
    if isinstance(oper_postfix, str):
        oper_postfix = stringolist(list(oper_postfix))
    elif not isinstance(oper_postfix, stringolist):
        raise TypeError(f'первый аргумент должен быть str или stringolist, а встреченно {oper_postfix.__class__.__name__}')
    if not (group_args_seps is None or isinstance(group_args_seps, list)):
        raise TypeError(f'аргумент group_args_seps должен быть list или None, а не {group_args_seps.__class__.__name__}')
    oper_postfix_len = len(oper_postfix)

    if group_args_seps is None:
        def postfix_parser(exp: 'str|stringolist') -> 'exprtree|stringolist|None':
            """Обрабатывает строку."""
            if isinstance(exp, str):
                exp = stringolist(list(exp))
            elif not isinstance(exp, stringolist):
                raise TypeError(f'ожидался str или stringolist, а встреченно {exp.__class__.__name__}')
            if not exp.endswith(oper_postfix):
                return None
            arg = exp[:-oper_postfix_len]
            if len(arg) == 1 and isinstance(arg[0], exprtree):
                arg = arg[0]
            return exprtree(oper, [arg])

        def postfix_repres(args: list) -> str:
            """Создаёт строку математического выражения из списка аргументов операции."""
            if not isinstance(args, list):
                raise TypeError(f'должен быть подан list, а не {args.__class__.__name__}')
            if len(args) != 1:
        ...
        def postfix_parser(exp: 'str|stringolist') -> 'exprtree|stringolist|None':
            ...
            pos = 0
            while True:
                pos = res.find(oper_postfix, pos)
                if pos == -1:
                    break
                arg_pos = pos - group_args_seps_len
                if not (arg_pos >= 0 and
                        all((g := group_args_seps[n][0]) is None or
                            (isinstance((a := res[arg_pos + n]), exprtree) and
                             a.val == g)
                            for n in range(group_args_seps_len))):
                ...
                for n, (group, sep) in enumerate(group_args_seps):
                    ...
                    cur_obj.trees += cur_args
                    cur_obj.trees[0].append(len(cur_args))
                res = res[:arg_pos] + stringolist(cur_obj) + res[pos + oper_postfix_len:]
                computed = True
            if computed:
                if len(res) == 1:
                    res = res[0]
        ...
        def postfix_repres(args: list) -> str:
            ...
            for n, var_count in enumerate(args[0]):
                ...
                if sep is None:
                    ...
                    res += group.repres(sep.join(str(x) for x in vs))
            return res + oper_postfix.to_str()

    return postfix_parser, postfix_repres
...
def operPostfix(oper_name: str, oper_postfix: str, group_args_seps: list = None) -> operation:
    """Возвращает постфиксную операцию.
oper_name - имя операции;
oper_postfix - обозначение постфиксной части операции в строковом выражении;
group_args_seps - список пар из операций-групп и разделителей операций-групп, принимаемых в качестве аргумента (по-умолчанию: None)."""
    res = operation(oper_name)
    res.funcs_update(postfix_parser_and_repres(oper_postfix, res, group_args_seps))
    return res
...
if __name__ == '__main__':
    ...
    if True:
        ...
        fact = operation('factorial')
        fact.funcs_update(postfix_parser_and_repres( '!',
                                                     fact,
                                                     None ))
        ...
        simple = algebra('simple',
                         [x for x in group.values()] + [root, split, plus, minus, power, fact, sin, cos, int_num, variable],
                         [int_num, variable, sin, cos, root, fact, power, plus, minus, split] + [x for x in group.values()],
                         group['()'])
